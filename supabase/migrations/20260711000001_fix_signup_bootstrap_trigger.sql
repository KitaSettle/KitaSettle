-- Fix signup failure: handle_new_user runs without a JWT, so auth.uid() is null.
-- The previous bootstrap_user_account guard treated null uid as forbidden and rolled back auth.users inserts.
