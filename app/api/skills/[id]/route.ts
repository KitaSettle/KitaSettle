...
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  const item = await repos.skills.getSkill(userId, id);
  if (!item) return jsonError("Skill not found", 404);
...
export async function PATCH(request: Request, { params }: RouteParams) {
  ...
  const repos = await getServerRepositories();
  const item = await repos.skills.update(userId, id, body);
  if (!item) return jsonError("Skill not found", 404);
  return NextResponse.json(item);
}
...
export async function DELETE(_request: Request, { params }: RouteParams) {
  ...
  const { id } = await params;
  const repos = await getServerRepositories();
  const deleted = await repos.skills.delete(userId, id);
